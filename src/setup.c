...
VkResult vkxCreateInstance(
        ...
        VKX_LOCAL_MALLOC(sizeof(VkLayerProperties) * layerPropertyCount);
    {
        ...
        VkResult result = vkEnumerateInstanceLayerProperties(
            &layerPropertyCount, pLayerProperties);
    ...
    vkEnumerateInstanceExtensionProperties(
        NULL, &extensionPropertyCount, NULL);
    pExtensionProperties = VKX_LOCAL_MALLOC(
        sizeof(VkExtensionProperties) * extensionPropertyCount);
    {
        // Retrieve extension properties.
        VkResult result = vkEnumerateInstanceExtensionProperties(
            NULL, &extensionPropertyCount, pExtensionProperties);
        if (VKX_IS_ERROR(result)) {
            // Free layer properties.
            VKX_LOCAL_FREE(pLayerProperties);
    ...
    for (uint32_t requestedLayerIndex = 0;
         requestedLayerIndex < requestedLayerCount; requestedLayerIndex++) {
        const char* pRequestedLayerName =
            ppRequestedLayerNames[requestedLayerIndex];

        // Iterate layers.
        VkBool32 enabled = VK_FALSE;
        for (uint32_t layerIndex = 0; layerIndex < layerPropertyCount;
             layerIndex++) {
            const char* pLayerName = pLayerProperties[layerIndex].layerName;
    ...
    for (uint32_t requestedExtensionIndex = 0;
        ...
        for (uint32_t extensionIndex = 0;
             extensionIndex < extensionPropertyCount; extensionIndex++) {
    ...
    VkResult result =
        vkCreateInstance(&instanceCreateInfo, pAllocator, pInstance);
...
static VkBool32 isPhysicalDeviceLess(
    ...
    if (pRequestedFeatures) {
        ...
        for (size_t pos = 0;
             pos < sizeof(VkPhysicalDeviceFeatures) / sizeof(VkBool32);
             pos++) {
...
VkPhysicalDevice vkxSelectPhysicalDevice(
        ...
        VKX_LOCAL_MALLOC(sizeof(VkPhysicalDevice) * physicalDeviceCount);
    {
        ...
        VkResult result = vkEnumeratePhysicalDevices(
            instance, &physicalDeviceCount, pPhysicalDevices);
        if (VKX_IS_ERROR(result)) {
...
VkFormat vkxSelectFormat(
    ...
    for (uint32_t index = 0; index < candidateFormatCount; index++) {
        ...
        // Features suitable?
        if ((requestedFeatures & features) == requestedFeatures) {
            // Found.
            return format;
        }
    }
...
uint32_t vkxGetFormatTexelSize(VkFormat format) {
    ...
    if ((format >= VK_FORMAT_R4G4B4A4_UNORM_PACK16 &&
        ...
        (format >= VK_FORMAT_R8G8_UNORM && format <= VK_FORMAT_R8G8_SRGB) ||
...
static uint32_t findFamilyForCreateInfo(
    ...
    for (uint32_t familyIndex = 0; familyIndex < familyCount; familyIndex++) {
        ...
        if (familySupportsCreateInfo(
                physicalDevice, familyIndex, &pProperties[familyIndex],
                pCreateInfo)) {
        ...
        for (uint32_t familyIndex = 0; familyIndex < familyCount;
            ...
            if (familySupportsCreateInfo(
                ...
                uint32_t otherIndex = findFamilyForCreateInfo(
                    ...
                    ppCreateInfosUsed, ppCreateInfosUsed[familyIndex],
                    depth + 1);
...
static VkBool32 findFamilyForEachCreateInfo(
    ...
    pProperties =
        VKX_LOCAL_MALLOC(sizeof(VkQueueFamilyProperties) * familyCount);
    vkGetPhysicalDeviceQueueFamilyProperties(
        physicalDevice, &familyCount, pProperties);
    ...
    for (uint32_t createInfoIndex = 0; createInfoIndex < createInfoCount;
        ...
        uint32_t familyIndex = findFamilyForCreateInfo(
            physicalDevice, familyCount, pProperties, ppCreateInfosUsed,
    ...
    for (uint32_t createInfoIndex = 0; createInfoIndex < createInfoCount;
        ...
        const VkxDeviceQueueFamilyCreateInfo* pFamilyCreateInfo =
            &pCreateInfos[createInfoIndex];
        ...
        // Allocate queues.
        pFamily->pQueues = malloc(pFamily->queueCount * sizeof(VkQueue));
        ...
        pFamily->pCommandPools =
            calloc(pFamily->commandPoolCount, sizeof(VkCommandPool));
...
VkResult vkxCreateDevice(
    ...
    if (!findFamilyForEachCreateInfo(
            physicalDevice, pCreateInfo->queueFamilyCreateInfoCount,
            pCreateInfo->pQueueFamilyCreateInfos, pDevice->pQueueFamilies)) {
    ...
    VkDeviceQueueCreateInfo* pQueueCreateInfos = VKX_LOCAL_MALLOC(
    ...
    for (uint32_t familyIndex = 0; familyIndex < pDevice->queueFamilyCount;
        ...
        VkDeviceQueueCreateInfo queueCreateInfo = {
            ...
            .queueCount = pDevice->pQueueFamilies[familyIndex].queueCount,
            .pQueuePriorities =
...

    {
        ...
        VkResult result = vkCreateDevice(
            physicalDevice, &deviceCreateInfo, pAllocator, &pDevice->device);
    ...
    for (uint32_t familyIndex = 0; familyIndex < pDevice->queueFamilyCount;
        ...
        for (uint32_t queueIndex = 0; queueIndex < pQueueFamily->queueCount;
             queueIndex++)
            vkGetDeviceQueue(
                pDevice->device, pQueueFamily->queueFamilyIndex, queueIndex,
                pQueueFamily->pQueues + queueIndex);

        // Create command pools.
        for (uint32_t poolIndex = 0;
             poolIndex < pQueueFamily->commandPoolCount; poolIndex++) {
            ...
            VkResult result = vkCreateCommandPool(
                pDevice->device, &commandPoolCreateInfo, pAllocator,
        ...
        for (uint32_t familyIndex = 0; familyIndex < pDevice->queueFamilyCount;
            ...
            for (uint32_t poolIndex = 0;
                ...
                vkDestroyCommandPool(
                    pDevice->device, pQueueFamily->pCommandPools[poolIndex],
                    pAllocator);
            // Free command pools.
