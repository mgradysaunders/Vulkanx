...
VkResult vkxCreateDescriptorSetGroup(
    ...
    memset(pSetGroup, 0, sizeof(VkxDescriptorSetGroup));
    {
        ...
        for (uint32_t typeIndex = 0; typeIndex < 16; typeIndex++) {
        ...
        VkResult result = vkCreateDescriptorPool(
            device, &poolCreateInfo, pAllocator, &pSetGroup->pool);
...

    {
        ...
        VkResult result = vkCreateDescriptorSetLayout(
            device, pSetLayoutCreateInfo, pAllocator, &pSetGroup->setLayout);
    ...
    VkResult result = vkAllocateDescriptorSets(
        device, &setAllocateInfo, &pSetGroup->pSets[0]);
...
void vkxDestroyDescriptorSetGroup(
    ...
    if (pSetGroup) {
        ...
        vkDestroyDescriptorSetLayout(device, pSetGroup->setLayout, pAllocator);

        // Destroy descriptor pool.
        vkDestroyDescriptorPool(device, pSetGroup->pool, pAllocator);
...
VkResult vkxCreateDynamicDescriptorPool(
    ...
    VkResult result = vkCreateDescriptorPool(
        device, pPoolCreateInfo, pAllocator, &initialPool);
    // Create descriptor pool error?
    if (VKX_IS_ERROR(result)) {
        return result;
    }
    ...
    VkDescriptorPoolSize* pPoolSizes = (VkDescriptorPoolSize*)malloc(
        sizeof(VkDescriptorPoolSize) * pPoolCreateInfo->poolSizeCount);
    for (uint32_t poolSizeIndex = 0;
         poolSizeIndex < pPoolCreateInfo->poolSizeCount; poolSizeIndex++) {
        pPoolSizes[poolSizeIndex] = pPoolCreateInfo->pPoolSizes[poolSizeIndex];
    }
    ...
    pDynamicPool->pFullFlags = (VkBool32*)malloc(sizeof(VkBool32) * 4);
...
VkResult vkxAllocateDynamicDescriptorSets(
    ...
    VkDescriptorSet* pSets =
        (VkDescriptorSet*)VKX_LOCAL_MALLOC(sizeof(VkDescriptorSet) * setCount);

    uint32_t associatedPoolIndex = UINT32_MAX;
    for (uint32_t poolIndex = 0; poolIndex < pDynamicPool->poolCount;
         poolIndex++) {
        // Pool not full?
        if (pDynamicPool->pFullFlags[poolIndex] == VK_FALSE) {
            ...
            VkResult result =
                vkAllocateDescriptorSets(device, &setAllocateInfo, pSets);
    ...
    if (associatedPoolIndex == UINT32_MAX) {
        ...
        VkDescriptorPool pool = VK_NULL_HANDLE;
        {
            // Create descriptor pool.
            VkResult result = vkCreateDescriptorPool(
                device, &pDynamicPool->poolCreateInfo, pAllocator, &pool);
            // Create descriptor pool error?
            if (VKX_IS_ERROR(result)) {
                // Free descriptor set array.
                VKX_LOCAL_FREE(pSets);
        ...
        if (pDynamicPool->poolCapacity == pDynamicPool->poolCount) {
            ...
            pDynamicPool->pPools = (VkDescriptorPool*)realloc(
                pDynamicPool->pPools,
                sizeof(VkDescriptorPool) * pDynamicPool->poolCapacity);
            pDynamicPool->pFullFlags = (VkBool32*)realloc(
...
VkResult vkxFreeDynamicDescriptorSets(
    ...
    for (uint32_t setIndexBegin = 0; setIndexBegin < setCount;) {
        ...
        for (; setIndexEnd < setCount &&
               pDynamicSets[setIndexBegin].associatedPoolIndex ==
                   pDynamicSets[setIndexEnd].associatedPoolIndex;
             setIndexEnd++) {
        }
        ...
        VkResult result = vkFreeDescriptorSets(
            device,
            pDynamicPool
                ->pPools[pDynamicSets[setIndexBegin].associatedPoolIndex],
            setIndexEnd - setIndexBegin, &pSets[setIndexBegin]);
        ...
        // Flag as not full.
        pDynamicPool
            ->pFullFlags[pDynamicSets[setIndexBegin].associatedPoolIndex] =
            VK_FALSE;
...
void vkxCmdBindDynamicDescriptorSets(
    ...
    vkCmdBindDescriptorSets(
        commandBuffer, pipelineBindPoint, pipelineLayout, firstSet, setCount,
        pSets, dynamicOffsetCount, pDynamicOffsets);
    ...
    if (pDynamicPool) {
        ...
        for (uint32_t poolIndex = 0; poolIndex < pDynamicPool->poolCount;
            ...
            vkDestroyDescriptorPool(
                device, pDynamicPool->pPools[poolIndex], pAllocator);
        }
