...
VkResult vkxAllocateSharedMemory(
    ...
    VkMemoryPropertyFlags memoryTypePropertyFlags[VK_MAX_MEMORY_TYPES];
    {
        ...
        for (uint32_t memoryTypeIndex = 0; memoryTypeIndex < memoryTypeCount;
             memoryTypeIndex++) {
    ...
    for (uint32_t memoryTypeIndex = 0; memoryTypeIndex < memoryTypeCount;
        ...
        for (uint32_t memoryRequirementIndex = 0;
            ...
            if ((((uint32_t)1 << memoryTypeIndex) &
                 pMemoryRequirements[memoryRequirementIndex].memoryTypeBits) &&
                (memoryTypePropertyFlags[memoryTypeIndex] &
    ...
    for (uint32_t memoryRequirementIndex = 0;
        ...
        for (uint32_t memoryTypeIndex = 0; memoryTypeIndex < memoryTypeCount;
            ...
            if ((((uint32_t)1 << memoryTypeIndex) &
                ...
                uint32_t supportedRequirementCount =
                    memoryTypeInfos[memoryTypeIndex].supportedRequirementCount;
                // Update.
    ...
    for (uint32_t memoryRequirementIndex = 0;
        ...
        pSharedMemory->pMemoryViews[memoryRequirementIndex].offset =
            pMemoryTypeInfo->allocateInfo.allocationSize;

        // Initialize memory view size.
        pSharedMemory->pMemoryViews[memoryRequirementIndex].size =
    ...
    for (uint32_t uniqueMemoryIndex = 0;
         ...
         uniqueMemoryIndex++) {
        pSharedMemory->pUniqueMemories[uniqueMemoryIndex] = VK_NULL_HANDLE;
    }
    ...
    for (uint32_t memoryTypeIndex = 0; memoryTypeIndex < memoryTypeCount;
        ...
        if (uniqueMemoryIndex != UINT32_MAX) {
            ...
            VkResult result = vkAllocateMemory(
                device, &pMemoryTypeInfo->allocateInfo, pAllocator,
...
void vkxFreeSharedMemory(
    ...
    if (pSharedMemory) {
        for (uint32_t uniqueMemoryIndex = 0;
            ...
            // Free.
            vkFreeMemory(
                device, pSharedMemory->pUniqueMemories[uniqueMemoryIndex],
                pAllocator);
        }
        // Free.
