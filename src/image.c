...
VkResult vkxCreateImage(
...

    {
        ...
        VkResult result = vkCreateImage(
            device, pImageCreateInfo, pAllocator, &pImage->image);
...

    {
        ...
        vkGetImageMemoryRequirements(
            device, pImage->image, &memoryRequirements);
        ...
        VkResult result = vkAllocateMemory(
            device, &memoryAllocateInfo, pAllocator, &pImage->memory);
        if (VKX_IS_ERROR(result)) {
            // Destroy image.
            pImage->memory = VK_NULL_HANDLE;
            vkxDestroyImage(device, pImage, pAllocator);
...
void vkxDestroyImage(
    ...
    if (pImage) {
        // Destroy image.
        vkDestroyImage(device, pImage->image, pAllocator);

        // Free memory.
        vkFreeMemory(device, pImage->memory, pAllocator);
...
VkResult vkxCreateImageGroup(
    ...
    pImageGroup->pImages = (VkImage*)malloc(sizeof(VkImage) * imageCount);
    ...
    for (uint32_t imageIndex = 0; imageIndex < imageCount; imageIndex++) {
        // Create image.
        VkResult result = vkCreateImage(
            device, &pImageCreateInfos[imageIndex], pAllocator,
            &pImages[imageIndex]);
        if (VKX_IS_ERROR(result)) {
        ...
        vkGetImageMemoryRequirements(
            device, pImages[imageIndex], &pMemoryRequirements[imageIndex]);
    }

    {
        // Allocate shared memory.
        VkResult result = vkxAllocateSharedMemory(
            physicalDevice, device, imageCount, pMemoryRequirements,
            pMemoryPropertyFlags, pAllocator, &pImageGroup->sharedMemory);
        if (VKX_IS_ERROR(result)) {
            // Free memory requirements.
            VKX_LOCAL_FREE(pMemoryRequirements);
            // Destroy image group.
    ...
    // Bind memory.
    VkxDeviceMemoryView* pMemoryViews = pImageGroup->sharedMemory.pMemoryViews;
    for (uint32_t imageIndex = 0; imageIndex < imageCount; imageIndex++) {
        VkResult result = vkBindImageMemory(
            device, pImages[imageIndex], pMemoryViews[imageIndex].memory,
...
VkResult vkxCreateDefaultImageView(
    ...
    // Is 1D image with more than 1 array layer?
        ...
        pImageCreateInfo->imageType == VK_IMAGE_TYPE_1D &&
        pImageCreateInfo->arrayLayers > 1) {
        viewType = VK_IMAGE_VIEW_TYPE_1D_ARRAY;
    }
    // Is 2D image with more than 1 array layer?
    else if (
        pImageCreateInfo->imageType == VK_IMAGE_TYPE_2D &&
        pImageCreateInfo->arrayLayers > 1) {
    ...
    // Is depth/stencil format?
        ...
        pImageCreateInfo->format == VK_FORMAT_D16_UNORM_S8_UINT ||
    ...
    return vkCreateImageView(
        device, &imageViewCreateInfo, pAllocator, pImageView);
}
...
VkResult vkxCreateDefaultImageViews(
    ...
    for (uint32_t imageIndex = 0; imageIndex < imageCount; imageIndex++) {
        ...
        result = vkxCreateDefaultImageView(
            device, pImages[imageIndex], &pImageCreateInfos[imageIndex],
            pAllocator, &pImageViews[imageIndex]);
        if (VKX_IS_ERROR(result)) {
            pImageViews[imageIndex] = VK_NULL_HANDLE;
            vkxDestroyImageViews(device, imageCount, pImageViews, pAllocator);
            break;
        }
    }
...
void vkxDestroyImageViews(
    ...
    if (pImageViews) {
        ...
        for (uint32_t imageViewIndex = 0; imageViewIndex < imageViewCount;
             imageViewIndex++)
            vkDestroyImageView(
                device, pImageViews[imageViewIndex], pAllocator);
        // Nullify.
...
VkResult vkxCmdTransitionImageLayout(
    ...
    default: return VK_ERROR_INITIALIZATION_FAILED; break;
    }
...
VkResult vkxTransitionImageLayout(
    ...
    // Transition image layout.
    VkResult result = vkxCmdTransitionImageLayout(
...
VkResult vkxGetImageData(
...

    {
        ...
        VkResult result = vkxCopyImageToBuffer(
            device, queue, commandPool, image, pImageDataAccess->layout,
            stagingBuffer.buffer, 1, &region, pAllocator);
    ...
    VkResult result = vkMapMemory(
        device, stagingBuffer.memory, 0, pImageDataAccess->size, 0,
        &pStagingBufferData);
...
VkResult vkxSetImageData(
    ...
    VkBufferImageCopy region = {
        ...
        .imageOffset = pImageDataAccess->offset,
        .imageExtent = pImageDataAccess->extent};
    VkResult result = vkxCopyBufferToImage(
        device, queue, commandPool, stagingBuffer.buffer, image,
        pImageDataAccess->layout, 1, &region, pAllocator);
