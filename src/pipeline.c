...
static VkGraphicsPipelineCreateInfo convertCreateInfo(
    ...
    VkPipelineVertexInputStateCreateInfo vertexInputState = {
        ...
        .pVertexBindingDescriptions = pCreateInfo->pInputState->pBindings,
        .vertexAttributeDescriptionCount =
            pCreateInfo->pInputState->attributeCount,
        .pVertexAttributeDescriptions = pCreateInfo->pInputState->pAttributes};
    ...
    if (pCreateInfo->pInputState->patchControlPoints > 0) {
        VkPipelineTessellationStateCreateInfo tessellationState = {
            ...
            .flags = 0,
            .patchControlPoints =
                pCreateInfo->pInputState->patchControlPoints};
    ...
    if (pCreateInfo->viewportCount > 0) {
        VkRect2D* pScissors =
            malloc(sizeof(VkRect2D) * pCreateInfo->viewportCount);
        // Viewport scissors provided?
        if (pCreateInfo->pViewportScissors) {
            // Copy.
            memcpy(
                pScissors, pCreateInfo->pViewportScissors,
                sizeof(VkRect2D) * pCreateInfo->viewportCount);
        }
        else {
            // Copy from viewports.
            for (uint32_t viewportIndex = 0;
                 viewportIndex < pCreateInfo->viewportCount; viewportIndex++) {
    ...
    VkPipelineRasterizationStateCreateInfo rasterizationState = {
        ...
        .pNext = NULL,
        .flags = 0,
        .depthClampEnable = pCreateInfo->pDepthState
                                ? pCreateInfo->pDepthState->depthClampEnable
                                : VK_FALSE,
        .rasterizerDiscardEnable =
            pCreateInfo->pInputState->rasterizerDiscardEnable,
        .polygonMode = pCreateInfo->pInputState->polygonMode,
        .cullMode = pCreateInfo->pInputState->cullMode,
        .frontFace = pCreateInfo->pInputState->frontFace,
        .depthBiasEnable = pCreateInfo->pDepthState
                               ? pCreateInfo->pDepthState->depthBiasEnable
                               : VK_FALSE,
        .depthBiasConstantFactor =
            pCreateInfo->pDepthState
                ? pCreateInfo->pDepthState->depthBiasConstantFactor
                : 0.0f,
        .depthBiasClamp = pCreateInfo->pDepthState
                              ? pCreateInfo->pDepthState->depthBiasClamp
                              : 0.0f,
        .depthBiasSlopeFactor =
            pCreateInfo->pDepthState
                ? pCreateInfo->pDepthState->depthBiasSlopeFactor
                : 0.0f,
    ...
    if (pCreateInfo->pMultisampleState ||
        ...
        VkPipelineMultisampleStateCreateInfo multisampleState = {
            ...
            .minSampleShading =
                pCreateInfo->pMultisampleState
                    ? pCreateInfo->pMultisampleState->minSampleShading
                    : 0.0f,
            .pSampleMask =
                pCreateInfo->pMultisampleState &&
                        pCreateInfo->pMultisampleState->sampleMaskEnable
                    ? &pCreateInfo->pMultisampleState->sampleMask[0]
                    : NULL,
    ...
    if (pCreateInfo->pDepthState || pCreateInfo->pStencilState) {
        ...
        VkPipelineDepthStencilStateCreateInfo depthStencilState = {
            ...
            .depthWriteEnable =
                pCreateInfo->pDepthState
                    ? pCreateInfo->pDepthState->depthWriteEnable
                    : VK_FALSE,
            .depthCompareOp = pCreateInfo->pDepthState
                                  ? pCreateInfo->pDepthState->depthCompareOp
                                  : (VkCompareOp)0,
            .depthBoundsTestEnable =
                pCreateInfo->pDepthState
                    ? pCreateInfo->pDepthState->depthBoundsTestEnable
                    : VK_FALSE,
            .stencilTestEnable =
                pCreateInfo->pStencilState ? VK_TRUE : VK_FALSE,
            .front = pCreateInfo->pStencilState
                         ? pCreateInfo->pStencilState->front
                         : defaultStencilOp,
            .back = pCreateInfo->pStencilState
                        ? pCreateInfo->pStencilState->back
                        : defaultStencilOp,
            .minDepthBounds = pCreateInfo->pDepthState
                                  ? pCreateInfo->pDepthState->minDepthBounds
                                  : 0.0f,
            .maxDepthBounds = pCreateInfo->pDepthState
                                  ? pCreateInfo->pDepthState->maxDepthBounds
                                  : 0.0f};
    ...
    VkPipelineColorBlendStateCreateInfo colorBlendState = {
        ...
        .blendConstants = {
            pCreateInfo->blendConstants[0], pCreateInfo->blendConstants[1],
            pCreateInfo->blendConstants[2], pCreateInfo->blendConstants[3]}};
...
VkResult vkxCreateGraphicsPipelines(
    ...
    VkGraphicsPipelineCreateInfo* pActualCreateInfos = VKX_LOCAL_MALLOC(
    ...
    for (uint32_t createInfoIndex = 0; createInfoIndex < createInfoCount;
         createInfoIndex++) {
        VkGraphicsPipelineCreateInfo* pActualCreateInfo =
            pActualCreateInfos + createInfoIndex;
    ...
    VkResult result = vkCreateGraphicsPipelines(
        device, pipelineCache, createInfoCount, pActualCreateInfos, pAllocator,
        pPipelines);
